...
mod tests {
    use {
        crate::{
            ...
            borrow::Cow,
            xml::{
                Entry, Feed, Parser, Parser2, ParserError, SkipDays, SkipHours,
                tests::{test_parser, test_parser_2},
            },
        },
    ...
    pub fn test_parser_ns_2<'src, T>(parser: &T, input: &'src str) -> Result<(), ParserError>
    where
        T: for<'alloc> Parser2<'alloc, 'src, Bump>,
    {
        with_bump(|alloc| {
            test_parser_2(
                parser,
